      ...
      @if(count($zahtevi)!=0)
        ...
        <div class="card mb-3">
            <div class="row g-0">
              ...
              <div class="col-md-8">
                <div class="card-body">
                  <h5 class="card-title">{{$zahtev->imeLjubimca}}</h5>
                  <p class="card-text">
                      Datum: {{$zahtev->datum}}<br>
                      Rasa: {{$zahtev->rasa}} <br>
                      Starost: {{$zahtev->starost}} <br>
                      Vlasnik: {{$zahtev->imeKorisnika}} <br>
                  </p>
                </div>
                <a href="{{ route('prihvatiZahtev', ['id' => $zahtev->id ]) }}">
                  <button type="submit" class="btn btn-dark" style="margin-left: 2.5%;" onclick="prihvatiParenje()">
                    Prihvati</button>
                </a>
                <a href="{{ route('odbiZahtev', ['id' => $zahtev->id ]) }}">
                    <button type="button" class="btn btn-dark" style="margin-left: 2.5%;" onclick="odbiParenje()">
                      Odbi</button>
                </a>
                <a href="{{ route('profilId', $zahtev->idKorisnika) }}">
      ...
      @else
        <div class="d-flex justify-content-center">
          <div class="alert alert-light" role="alert">
            <h5>Nemate zahteve za šetnje</h5>
          </div>
        </div>
       @endif
